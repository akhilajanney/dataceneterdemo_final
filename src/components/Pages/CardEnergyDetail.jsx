...
export default class CardEnergyDetail extends Component {
  ...
  rackChartData = (macid) => {
    ...
    let value = [];
    axios({ method: 'GET', url: '/api/rack/average?id=' + macid + "&key=rackenergymax" })
      .then((response) => {
        ...
        if (data.length !== 0 && response.status === 200) {
          ...
          this.setState({
            macId: macid, message1: "No Energy Data Found", error: false,
            series: [{ name: "Energy(kWh)", data: [[milliseconds1]] }]
          });
        }
  ...
  assetChartData = (tagid) => {
    ...
    var date1 = new Date();
    var milliseconds1 = date1.getTime();
    axios({ method: "GET", url: "/api/track?id=" + tagid +"&key=assetenergymax"})
      .then((response) => {
        ...
        if (data.length !== 0 && response.status === 200) {
          ...
          this.setState({
            macId: tagid, message1: "No Energy Data Found", error: false,
            series: [{ name: "Energy(kWh)", data: [[milliseconds1]] }]
          });
        }
      })
      .catch((error) => {
        ...
        if (error.response.status === 403) {
          ...
          this.setState({
            error: true,
            message1: "Error occurred. Please try again.",
          });
        }
      })
  };
